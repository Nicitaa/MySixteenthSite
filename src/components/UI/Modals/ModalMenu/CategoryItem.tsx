    ...
    <>
      <div className="flex flex-row justify-between">
        <div className="modal-list-text-info-container">
          ...
          <div className="modal-list-text-info-body subTitle">
            {item.ingredients.map((ingredient) => (
              <span key={ingredient.id}>
                {item.ingredients.indexOf(ingredient) != item.ingredients.length - 1 ? (
                  <span>
                    <a
                      className="modal-list-item-subTitle"
                      onClick={() => {
                        /* open dropdown */
                      }}>
                      {ingredient.label}
          ...
          <div className="modal-list-text-info-footer">
            <motion.a className="add-to-cart hover:cursor-pointer" whileHover={{ scale: 0.975 }}>
              Add to cart
            </motion.a>
            <div className="list-item-info" onMouseOver={() => setIsInfo(true)} onMouseLeave={() => setIsInfo(false)}>
              <motion.div>
                <AiOutlineInfoCircle />
              </motion.div>
            </div>
      ...
      <div
        className={`MobileL:grid hidden grid-cols-2 gap-2`}
        style={{
        ...
        {item.ingredients.map((ingredient) => (
          <div key={ingredient.id}>
            <span className="flex flex-row items-center justify-between gap-x-2">
              ...
              <img className="w-[32px] h-[32px] object-cover" src={ingredient.imgSrc} width={32} height={32} />
            </span>
          </div>
        ))}
      </div>
